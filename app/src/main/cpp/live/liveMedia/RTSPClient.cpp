...
void RTSPClient::sendDummyUDPPackets(MediaSession &session, unsigned numDummyPackets) {
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "TAG", "sendDummyUDPPackets");
...
static void copyUsernameOrPasswordStringFromURL(char *dest, char const *src, unsigned len) {
    ...
    while (len > 0) {
        int nBefore = 0;
        int nAfter = 0;
        __android_log_print(ANDROID_LOG_DEBUG, "TAG", "copyUsernameOrPasswordStringFromURL");
    ...
    do {
        ...
        if (addresses.numAddresses() == 0) {
            envir().setResultMsg("Failed to find network address for \"",
                                 parseBuffer, "\"");
            __android_log_print(ANDROID_LOG_DEBUG, "TAG", "Failed to find network address for %s",
                                parseBuffer);
            break;
        }
        ...
        if (nextChar == ':') {
            ...
            if (sscanf(++from, "%d", &portNumInt) != 1) {
                ...
                __android_log_print(ANDROID_LOG_DEBUG, "TAG", "No port number follows %d",
                                    portNumInt);
                break;
            }
            if (portNumInt < 1 || portNumInt > 65535) {
                envir().setResultMsg("Bad port number");
                __android_log_print(ANDROID_LOG_DEBUG, "TAG", "Bad port number");
                break;
            }
...
int RTSPClient::openConnection() {
    do {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "TAG", "RTSPClient >> openConnection");
        ...
        if (fVerbosityLevel >= 1) {
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "TAG",
                                "Created new TCP socket ':%d' for connection", fInputSocketNum);
        }
        // Connect to the remote endpoint:
        int connectResult = connectToServer(fInputSocketNum, destPortNum);
        if (connectResult < 0) {
            __android_log_print(ANDROID_LOG_DEBUG, "TAG",
                                "connectResult = %d, break;", connectResult);
            break;
        }
        else if (connectResult > 0) {
            if (fTLS.isNeeded) {
                ...
                if (connectResult > 0 && fVerbosityLevel >= 1) {
                    __android_log_print(ANDROID_LOG_DEBUG, "TAG", "...TLS connection completed");
            ...
            if (connectResult > 0 && fVerbosityLevel >= 1) {
                __android_log_print(ANDROID_LOG_DEBUG, "TAG", "...local connection opened");
...
int RTSPClient::connectToServer(int socketNum, portNumBits remotePortNum) {
    ...
    if (fVerbosityLevel >= 1) {
        envir() << "Connecting to " << AddressString(fServerAddress).val() << ", port "
                << remotePortNum << " on socket " << socketNum << "...\n";
    }
...
void RTSPClient::connectionHandler1() {
    ...
    do {
        ...
        if (getsockopt(fOutputSocketNum, SOL_SOCKET, SO_ERROR, (char *) &err,
                       reinterpret_cast<socklen_t *>(&len)) < 0 || err != 0) {
...
::createNew(UsageEnvironment &env, onRTSPClientCreationFunc *creationFunc, Port ourPort,
    ...
    return new HandlerServerForREGISTERCommand(env, creationFunc, ourSocketIPv4, ourSocketIPv6,
                                               ourPort, authDatabase, verbosityLevel,
                                               applicationName);
}
